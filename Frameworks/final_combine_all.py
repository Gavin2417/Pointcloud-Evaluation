...
# Confidence from Randla's softmax output
...
# Confidence from local coverage, support, and consistency
...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            p = all_probs                                   
            sem_risk_point = labels.astype(float)           

            # Get confidence from softmax entropy (0..1, higher is more confident)
            sem_conf_point = 1.0 - softmax_entropy(p)       
            ...
            # Calculate risk grids and fuse them with edge preservation
            ...
            # Get the number of points per cell for confidence weighting and inflation penalty
            ...
            # Normal confidence-weighted average 
            ...
            # Risk inflation penalty (uncertainty: push toward worst case)
            max_conf = np.nanmax(np.stack([geom_conf_grid, sem_conf_grid]), axis=0)
            penalty = (1.0 - max_conf) * STEP_config['MAX_RTSK_VALUE'] * 0.1 
            ...
            # Carry over previous values inside persistent annulus into current NaNs
            ...
            with np.errstate(invalid='ignore'):
                ...
                # Distance transform from high-risk cells (treating non-finite as high if configured) to create a smooth inflation penalty 
                # that decays with distance from the nearest high-risk blob, with a hard cutoff at the specified radius
                dist_cells = distance_transform_edt(~high_mask)   
                dist_m = dist_cells * STEP_config['grid_resolution']
                radius_m   = float(STEP_config.get('inflation_radius', 0.8))   
                boundary_p = float(STEP_config.get('halo_boundary_level', 0.7))  
                profile    = STEP_config.get('inflation_profile', 'smoothstep')   
                ...
                # Keep fades tied to boundary, not absolute zero
            ...
            now = time.time()
            dt_meas = max(min(now - prev_t, 0.2), 0.05)  
            prev_t = now
