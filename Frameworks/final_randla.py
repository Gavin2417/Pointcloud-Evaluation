...
# Utility to compute path cost given a cost map used for A* planning, treating out-of-bounds as very expensive. This is used to compare old vs new paths for hysteresis.
   ...
   # Map setup:
    ...
    try:
        while True:
            ...
            labels, probs = seg.segment(points_world)
            for p, lab in zip(points_world, labels):
            ...
            mean_ground, _, _, _ = binned_statistic_2d(
                ground_pts[:,0], ground_pts[:,1], ground_pts[:,2], statistic='mean', bins=[x_edges, y_edges]
            )

            # Calculate risk grid            
            risk_grid = interpolate_in_radius(mean_ground, STEP_config['interpolate_radius'])
            ...
            # Default goal index is the start index (no movement) if no valid temp goal, 
            ...
            if STEP_config['visualize'] and (now_viz - last_viz_t) >= STEP_config.get('viz_dt', 0.25):
                ...
                if STEP_config['Capturing']:
                    path = os.path.join(base, "record/finalrandla_4", args.name)
    ...
    os.chdir(base)
    stats_file = os.path.join(base, "record/finalrandla_stats_4.json")
    all_runs = []
