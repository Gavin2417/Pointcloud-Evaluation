...
from utils.config import Config50labels as cfg
...
class SemanticKITTI(torch_data.Dataset):
    ...
    def crop_pc(points, labels, search_tree, pick_idx):
        # Pick center and clamp k
        center = points[pick_idx].reshape(1, -1)
        num_pts = points.shape[0]
        k = min(cfg.num_points, num_pts)

        # Get up to k neighbors
        neigh_idx = search_tree.query(center, k=k)[1][0]

        # Pad if fewer than cfg.num_points
        if k < cfg.num_points:
            pad = cfg.num_points - k
            extra = np.random.choice(neigh_idx, pad, replace=True)
            neigh_idx = np.concatenate([neigh_idx, extra])

        # Shuffle into exactly cfg.num_points
        select_idx = DP.shuffle_idx(neigh_idx)
