    ...
    def __init__(self, config):
        ...
        for j in range(self.config.num_layers):
            if j < self.config.num_layers - 1:
                # “standard” case: match up with the encoder’s (-j-2) layer
                prev_d_out = d_out    # d_out carries over from the previous iteration
                idx        = -j - 2   # e.g. j=0 → -2, j=1 → -3, etc.
                d_in  = prev_d_out + 2 * self.config.d_out[idx]
                d_out = 2 * self.config.d_out[idx]
            else:
                # last decoder layer (j == num_layers-1): use config.d_out[0]
